...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate:
        eval_dict = {}
        for index_idx in range(11):
            acc1, acc5, _ = validate(val_loader, model, criterion, args, epoch=index_idx)
            eval_dict[f"head_{index_idx:-2d}"] = {"acc1": acc1, "acc5": acc5}
        acc1, acc5, _ = validate(val_loader, model, criterion, args, epoch=-1)
        eval_dict[f"head_last"] = {"acc1": acc1, "acc5": acc5}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for param in model.parameters():  # freeze all
            ...
            param.grad = None
        for param in model.module.transformerheads[epoch].parameters():  # unfreeze nth head
